...
def submit(x,y):
    ...
    if(c2==1 or c3==1 or c5==1):
        ...
        if(x==0):
            c.execute("insert into donation values("+pid+","+c1+","+c2+","+c3+","+c4+","+c5+","+c6+")")
            c.commit()
        tkinter.messagebox.showinfo("SUCCESS",  "Donation done")
    report()
def search():
    global intro1_f,report_f,search_f,search_i,donations,search_home_i,search_report_i,search_search_i,search_user_i,donate_b_i,search_donate_direct_i,donate_f,name
    intro1_f.destroy()
    report_f.destroy()
    search_f.destroy()
    donate_f.destroy()
    ...
    y=204
    for d in ls:
        donations.append(donate(d[0],name,d[2],d[3],x,y))
        x=x+514
        print(d)
        if(x>=1660):
            x=302
...
def donated(x,y=1):
    ...
    insert_b = Button ( donate_c, width=10,height=1,font=('semi bold',24,'normal'),bg='#C72542',text='SUBMIT',fg='#FFFFFF',command=lambda:submit(1,pid))
